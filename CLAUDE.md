...
The project is a **thin wrapper** around catalyst. It pins the maintained fork (`AndriyKalashnykov/catalyst`); the project's own code is **not** a fork of catalyst — only a release tag is held in `CATALYST_REF`. The pieces:

1. **`CATALYST_REF`** pins a specific release **tag** (e.g. `v1.3.0`) of the maintained fork. Renovate tracks `AndriyKalashnykov/catalyst` tags via a `github-tags` custom manager (see `renovate.json`) — the fork tags releases but does not cut formal GitHub Releases, so `github-tags` (not `github-releases`) is the correct datasource. Repo-wide automerge is on (`automergeType: pr`), but the `CATALYST_REF` package rule carves out `automerge: false` — bumps to catalyst are API-affecting and need human review.
2. **`scripts/fetch-catalyst.sh`** clones the pinned ref into `vendor/catalyst/`, runs `npm ci`, transiently installs `typescript@~5.7` (`--no-save`) when `tsc` isn't already in `node_modules/.bin` — upstream catalyst's build script is `tsc` but typescript isn't in its devDependencies — runs `npm run build` (tolerated if it exits non-zero since tsc emits `dist/` anyway), then **wipes `node_modules/` and reinstalls with `--omit=dev --ignore-scripts`** (plain `npm prune --omit=dev` leaves transitive devDep trees intact, shipping HIGH CVEs into the image). Idempotent: skips the whole block when the vendored checkout already matches `CATALYST_REF` and has a `dist/`. The Dockerfile's `catalyst-builder` stage mirrors the same defense.
...
- `CATALYST_REF` — pinned release **tag** of the maintained fork (`AndriyKalashnykov/catalyst`; upstream `localgod/catalyst` is inactive). Single source of truth; the Makefile reads it via `$(shell tr -d '[:space:]' < CATALYST_REF)`, the Dockerfile consumes it as a `--build-arg`, and Renovate tracks the fork's tags via a `github-tags` custom manager. The regex captures the tag as `currentValue` (`^(?<currentValue>v?\d[\w.+-]*)`); a SHA-shaped value would NOT match — keep `CATALYST_REF` a `vX.Y.Z` tag.
...
- [x] ~~**Docker-image batch-mode is not e2e-asserted**~~ — RESOLVED 2026-05-15: `make e2e-batch` added (bind-mount batch convert + assert `<mxGraphModel`, host-owned non-root write-back, fresh mtime) and wired into the CI `e2e` job directly (bypassing `make ci-run`/act, where nested bind mounts are unreliable). The stdin-mode `make e2e` and the integration suite still cover their layers.
