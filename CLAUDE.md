...
The entire `docker` job is gated to version tags (`if: startsWith(github.ref, 'refs/tags/')`), so on PRs and `main` pushes it is skipped — the Trivy image scan, `--version` smoke test, and multi-arch build validation run only at release-tag time. Dockerfile breakage is still caught on PRs by the `e2e` job, which builds and runs the image. Within the job, the publish/sign steps (registry push + cosign sign + manifest verify) keep their step-level `if: startsWith(github.ref, 'refs/tags/')` guards as defense-in-depth.
...
- **pnpm-only.** `package.json` pins pnpm via its `packageManager` field (Renovate-tracked — see `package.json` for the current version). Never run `npm install` at the wrapper root — it will write `package-lock.json` and cause drift. (Inside `vendor/catalyst/`, npm is used because catalyst itself uses `package-lock.json` upstream.)
